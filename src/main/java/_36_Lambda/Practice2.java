...
public class Practice2 {
    public static void main(String[] args) {
        ...
        // 이름이 3글자 초과인 사람이면서, 성인인 사람들만 뽑아서 저장
        // 뽑는 것은 PersonChecker를 implement 받은 익명 클래스의 객체로 구현(람다)
        List<Person> results = new ArrayList<>();
        PersonChecker longNameChecker = p -> getName().length() > 3;
        ...
        // 이름의 두 번째 글자를 *로 바꿔주는 람다 코드 작성
        // Person 객체를 매개변수로 받아서 이름을 변경하고
        // 이름이 변경된 Person 객체를 리턴해 준다.
        Modifier<Number, S> makeMasking = p -> {
            String name = p.getName();
            if(name.length() >= 2) {
                // 홍길동 -> 홍*동
                // 박혁거세 -> 박*거세
                String maskedName
                        // charAt(index) : 문자열에서 한 글자만 추출할 때 사용
                        = name.substring(0, 1)
                        + "*"
                        + name.substring(2);
                // Person 객체를 리턴해 줘야 하는데,
                // 1. 매개변수로 받은 Person 객체를 조작 (setter 사용)
                // 2. 새로운 Person 객체를 만들어서 리턴 (new 사용)
        ...
        // Modifier를 이용
        // 미성년자(age가 20세 미만)의 이름을 "비공개"로 바꾸는 Modifier 작성
        List<Person> minors = new ArrayList<>();
        // 바꾼 Person 객체들을 minors에 담아서 출력
        Modifier<Number, S> hideMinorNames = p -> {
            if(p.getAge() < 20) {
                // 20 미만일 때
        ...
        System.out.println(people); // setter로 구현하면 원본 보존이 힘듦
