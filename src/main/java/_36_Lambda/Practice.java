...
public class Practice {
    public static void main(String[] args) {
        // List.of()로 List 생성하면 불변객체로 생성됨 (변경 불가)
        List<String> customerNames
                = List.of("홍길동", "김길동", "박길동", "최길동");

        // 람다로 customerNames에 담긴 이름들 뒤에 +"님_을 붙이는 코드
        ...
        StringProcessor makeNim = s -> s + "님";
            ...
            // StringProcessor를 재정의해서 람다식으로
            // lowers에 있는 소문자들을 모두 대문자로 만들어서 출력해 주세요
            // 최종 uppers를 출력해 주세요
            // 문자열.toUpperCase() 사용하면 됨
            ...
            for (String lower : lowers) {
                String upper = makeUpper.process(lower);
                uppers.add(upper);
            }

            List<String> names
                    = List.of("김풍", "침착맨", "이순신", "홍길동", "을지문덕", "페이커", "손", "쵸비");
