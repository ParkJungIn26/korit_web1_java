...
public class LambdaMain {
    ...
    public static void main(String[] args) {
        ...
        FP를 지원한다면 가능해야 하는 것
        1. 메서드를 변수에 할당할 수 있어야 한다.
        add = x, y -> return x + y;
        2. 리턴으로도 함수가 전달될 수 있어야 한다.
        calc = x, fn -> return fn(x);

        하지만, 자바는 메서드만 단독으로 존재할 수 없음.
        메서드가 객체가 안 된다면, 객체처럼 보이면 되지 않을까
        -> 메서드 하나만 가지는 클래스를 만들면 되지 않을까?

        메서드를 값처럼 변수에 할당 -> 익명 클래스 객체를 사용하자
        ...
        // 익명클래스
        // new로 생성되는 객체 -> 익명1, 익명2 같은 익명클래스 타입
        // 변수타입은 Calculator로서 업캐스팅 상태로 선언되어 있음
        Calculator add = new Calculator() {
            @Override
            public int calculate(int a, int b) {
                return a + b; // 사실상 핵심 로직은 단 한 줄
            }
        };

        // multi의 실제 타입은 add와는 다른 클래스 타입이다.
        // 둘 다 익명클래스 객체이지만, 서로 다른 익명클래스 객체인 것
        ...
        // 메서드 시그니처를 특정할 수 있다 -> 매개변수 타입도 특정 가능
        // + 한 줄로 리턴이 가능하면, 중괄호 + return 키워드 생략 가능
        ...
        // 익명 객체는 정의와 동시에 생성 -> 매개변수로 익명클래스를 받는 경우
