...
public class WaterBottle {
    ...
    // OOP에서 중요하게 여기는 가치 중 하나는 생성자를 통한 값 검증이다.
    public void WaterBottle(int currentWater) {
        if (currentWater < 0) {
            System.out.println("물의 양은 음수일 수 없습니다.");
            return; // 생성자에서는 예외를 던지는 게 올바른 방법이다.
        }
    ...
    public int getCurrentWater() {
        ...
        public void fill(int amount) {
            ...
            if (result > 1000) {
                System.out.println("물병의 최대 용량은 1000입니다. 현재 양을 초과합니다.");
                return;
            }
...
/*
    ...
    메서드 :
        - void fill(int amount)
            1. amount 값을 검증 (0보다 큰지)
            2. 총량은 1000ml를 넘으면 안 된다.
            3. 유효하면 "물을 채웠습니다." 출력
        - void drink(int amount)
            1. amount 값을 검증 (0보다 큰지)
            2. 현재 물의 양보다 더 많은 양을 마실 수는 없음
            3. 유효하면 "물을 마셨습니다." 출력
        - int getCurrentWater()
            -> 현재 물의 양 리턴
     */
