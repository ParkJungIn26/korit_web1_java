...
public class BookRepository {
    Book[] books; // book들을 저장할 배열(DB 대용)
    ...
    // 주소가 없는 경우가 있음 (초기화가 안 된 것)
    // 저장된 주소가 없습니다. 를 표현하는 값
    // NullPointerException -> 참조자료형은 .으로 참조할 수 있는데
    // 초기화가 안 되면 null.

    int getEmptyIndex() {
        for(int i = 0; i < books.length; i++) {
            if (books[i] == null) { // books의 i번째가 비어 있으면
    ...
    Book[] getBookDatas() {
        int count = 0;
        // 1. null이 아닌 도서의 개수를 센다 (count 활용)
        for (int i = 0; i < books.length; i++) {
            if (books[i] != null) {
                count++;
            }
