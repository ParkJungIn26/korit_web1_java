...
// 2. T는 Object까지 업캐스팅 되지 않고, Animal까지만 업캐스팅 된다. (컴파일러가 자동으로)
public class AnimalHostpital<T extends Animal> {

    // 필드로 Animal을 상속받는 객체들을 제너릭으로 받고자 한다.
    private T animal;

    // 필드에 있는 Animal 상속객체들의 sound() 호출을 해야 하는데,
    // T로는 업캐스팅이 Object까지 이루어지기 때문에 sound() 메서드 호출 불가
    // 하지만 상한 경계를 설정해 주면, sound() 메서드가 있다는 걸 보장할 수 있어서
    // 컴파일러가 컴파일 검사를 통과시켜 준다.
