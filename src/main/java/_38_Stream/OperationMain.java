...
public class OperationMain {
    public static void main(String[] args) {
        ...
        // 문자(문자열) 정렬
        // 문자는 숫자로 변환할 수 있음
        // b가 a보다 더 큰 숫자로 변환됨 (사전에서 뒤에 출현하는 문자가 더 큰 숫자)
        ...
        boolean isOver2Nmaes = names.stream()
                .allMatch(name -> name.length() >= 2);
        // 결과가 모두 true면 true

        boolean noShortName = names.stream()
                .noneMatch(name -> name.length() < 2);
        // 만족하는 조건이 없어야 true (결과가 모두 false면 true)
        ...
        // 변수를 필드로 가지는 컨테이너 클래스

        // 5. reduce
        // numbers에 있는 모든 값의 총합을 구해보자
