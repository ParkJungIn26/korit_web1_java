...
public class Main {
    public static void main(String[] args) {
        ...
        // 실습
        // 1. 90점 이상 학생이 있는지 확인 - boolean 타입 출력 - anyMatch
        ...
        List<String> top3Names = students.stream()
                .sorted((str1, str2) -> str2.getScore() - str1.getScore())
                .limit(3)
                .map(student -> student.getName)
                .collect(Collectors.toList());

        // 3. "컴퓨터공학"과이면서 90점 이상인 학생 이름을 담은 리스트 출력
        // 필터링 -> 객체에서 문자열 변환 -> 모으기
        List<String> results = students.stream()
                .filter(student -> student.getMajor().equals("컴퓨터공학") && student.getScore() >= 90)
                .map(student -> student.getName())
                .collect(Collectors.toList());
        ...
        // 학생들의 전공을 중복없이 모두 추출해서 모은 리스트 출력
        // distinct -> 객체 상대로 distinct()를 적용하면 equals(), hashcode()로 비교
